...
# coding: utf-8
# COMMAND:
# python x2coco.py --dataset_type voc  --voc_anno_dir D:\Treat_for_data\Data\Annotations --voc_anno_list val.txt --voc_label_list label_list.txt  --voc_out_name coco_val.json

# hmy change for unuseable xml
import argparse
import glob
import json
import os
...
def voc_get_label_anno(ann_dir_path, ann_ids_path, labels_path):
    ...
    return dict(zip(labels_str, labels_ids)), ann_paths, ann_ids

#4
def voc_get_image_info( ann_ids, annotation_root, im_id):
    a = im_id
    filename = ann_ids[a]+'.jpg'
...
#2
def voc_xmls_to_cocojson(ann_ids,annotation_paths, label2id, output_dir, output_file):
    output_json_dict = {
        "images": [],
        "type": "instances",
        "annotations": [],
    ...
    for a_path in tqdm(annotation_paths):
        ...
        #3
        img_info = voc_get_image_info( ann_ids, ann_root, im_id)
...
#1
    if args.dataset_type == 'voc':
        ...
        label2id, ann_paths, ann_ids = voc_get_label_anno(
